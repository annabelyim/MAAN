...
for (r in 1: nrow(joined_data)) {
  count = 0
  data <- joined_data[r,]
  data_i <- data %>% select(! matches('.[_j]') & ! matches('title')) %>% unlist()
  data_j <- data %>% select(matches('.[_j]') & ! matches('title_j')) %>% unlist()
  for (col in 1:length(data_i)) {
    ...
    if (data_i[[col]] == data_j[[col]] & data_i[[col]] > 0 & data_j[[col]] > 0) {
      count = count + 1
    }
  }
...
plot(network_weights, 
     edge.width=0.005*E(network_weights)$weight,
     vertex.color="white",
     vertex.label.color="blue"
     )
title(main="Keywords Network Between Written Texts")
```
